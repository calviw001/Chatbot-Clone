  ...
  try {
    ...
    if (selectedChat.length === 0) {
      return res
        ...
        .json({
          message:
            "Cannot get messages from a chat that does not belong to the current user.",
        });
    }
  ...
  try {
    ...
    if (selectedChat.length === 0) {
      return res
        ...
        .json({
          message:
            "Cannot add a new message to a chat that does not belong to the current user.",
        });
    }
    ...
    const [messageCount] = await db.query(
      "SELECT COUNT(*) AS count FROM messages WHERE messages.chat_id = ?",
      [chatId],
    );
    if (messageCount[0].count === 1) {
      const newTitle = createNewChatTitle(content);
      await db.query("UPDATE chats SET title = ? WHERE chats.id = ?", [
        newTitle,
        chatId,
      ]);
    }
